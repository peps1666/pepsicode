"""Performance benchmark suite for pepsicode Python.
...
# Add parent directory to path to import pepsicode
import sys
sys.path.insert(0, str(Path(__file__).parent.parent))

from pepsicode.context_manager import estimate_tokens, estimate_message_tokens
...
def benchmark_terminal_rendering() -> list[BenchmarkResult]:
    ...
    from pepsicode.tui.transcript import render_transcript
...
def benchmark_token_estimation() -> list[BenchmarkResult]:
    ...
    test_cases = [
        ...
        ("Chinese only", "浣犲ソ涓栫晫" * 100),
        ("Mixed CJK/ASCII", "Hello 浣犲ソ World 涓栫晫 " * 50),
...
def run_all_benchmarks() -> list[BenchmarkResult]:
    ...
    print("=" * 80)
    print("pepsicode Python Performance Benchmark")
    print("=" * 80)
    print()
    ...
    for name, func in benchmarks:
        print(f"馃搳 Running {name} benchmarks...")
        try:
            results = func()
            all_results.extend(results)
            print(f"   鉁?{len(results)} benchmarks completed\n")
...
if __name__ == "__main__":
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("pepsicode Python Benchmark Results\n")
        f.write("=" * 80 + "\n\n")
        for r in results:
