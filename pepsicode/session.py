...
to allow pepsicode to save and restore conversation state across restarts.
"""
...
SESSIONS_DIR = PEPSI_CODE_DIR / "sessions"
...
def _session_index_file() -> Path:
    ...
    return PEPSI_CODE_DIR / "sessions_index.json"


def resolve_session_id(session_id: str) -> str | None:
    """Resolve a full session id from an exact id or a unique prefix."""
...
def load_session(session_id: str) -> SessionData | None:
    """Load a session from disk by full id or unique prefix. Returns None if not found."""
    resolved_id = resolve_session_id(session_id)
    if resolved_id is None:
        return None

    session_path = _session_file(resolved_id)
