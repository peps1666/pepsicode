...
def _run(input_data: dict, context) -> ToolResult:
    ...
    if error:
        return ToolResult(ok=False, output=f"鉂?{error}")
    ...
    try:
        if action == "explore":
            ...
            if not table:
                return ToolResult(
                    ok=False,
                    output="鉂?'table' parameter is required when action is 'schema'",
                )
            ...
            if table not in tables:
                return ToolResult(
                    ok=False,
                    output=f"鉂?Table '{table}' not found. Available tables: {', '.join(tables)}",
                )
            ...
            if not query:
                return ToolResult(
                    ok=False,
                    output="鉂?'query' parameter is required when action is 'query'",
                )
            ...
            if not query.strip().upper().startswith("SELECT"):
                return ToolResult(
                    ok=False,
                    output="鉂?Only SELECT queries are allowed for safety.",
                )
            ...
            try:
                ...
                return ToolResult(
                    ok=False,
                    output=f"鉂?Query failed: {e}\n\nQuery: {query}",
                )

            # Format results
            lines = ["\U0001f4ca Query Results", "=" * 60, ""]
