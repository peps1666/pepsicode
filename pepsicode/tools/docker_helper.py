...
def _run(input_data: dict, context) -> ToolResult:
    ...
    try:
        if action == "ps":
            ...
            if not success:
                return ToolResult(ok=False, output=f"鉂?Docker ps failed: {stderr}")
            ...
            if not success:
                return ToolResult(ok=False, output=f"鉂?Docker exec failed: {stderr}")
            ...
            if not success:
                return ToolResult(ok=False, output=f"鉂?Docker compose up failed: {stderr}")
            ...
            if not success:
                return ToolResult(ok=False, output=f"鉂?Docker compose down failed: {stderr}")
        ...
        return ToolResult(ok=False, output=f"鉂?Error: {e}")
