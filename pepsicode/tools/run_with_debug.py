    ...
    for file_path, line_num, func_name in frames:
        ...
        if abs_path.exists():
            try:
                ...
                # Get surrounding context (卤2 lines)
...
def _run(input_data: dict, context) -> ToolResult:
    ...
    try:
        ...
        return ToolResult(
            ok=False,
            output=f"鉂?Command timed out after {timeout} seconds\n\nCommand: {command}",
        )
    except FileNotFoundError:
        return ToolResult(
            ok=False,
            output=f"鉂?Command not found\n\nCommand: {command}",
        )
    except Exception as e:
        return ToolResult(
            ok=False,
            output=f"鉂?Execution error: {e}\n\nCommand: {command}",
        )
