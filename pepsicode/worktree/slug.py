...
def validate_slug(name: str) -> str | None:
    """Validate a worktree name. Returns an error message, or None if valid.

    Names may contain ``/`` to suggest grouping (e.g. ``feature/foo``);
    each segment must be a safe filename component.
    """
    if not name:
...
def flatten_slug(name: str) -> str:
    """Flatten a possibly-nested name into a single filesystem-safe directory name."""
    return name.replace("/", "+")
