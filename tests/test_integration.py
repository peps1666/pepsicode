"""End-to-end integration tests for pepsicode Python.

Tests the full pipeline: agent loop 鈫?model 鈫?tool execution 鈫?message flow,
...
class TestSessionIntegration:
    ...
    def test_session_save_load_roundtrip(self, tmp_workspace):
        ...
        # Cleanup
        session_path = PEPSI_CODE_DIR / "sessions" / "test-integration-001.json"
    ...
    def test_session_listing(self, tmp_workspace):
        ...
        # Cleanup
        for i in range(3):
            path = PEPSI_CODE_DIR / "sessions" / f"test-list-{i:03d}.json"
            if path.exists():
                path.unlink()
...
class TestTranscriptPoolIntegration:
    ...
    def test_pool_create_recycle_reuse(self):
        ...
        # Create new ones 鈥?should reuse from pool
...
@pytest.mark.skipif(
    ...
    reason="ANTHROPIC_API_KEY not set 鈥?skipping live API test",
)
class TestLiveAPI:
