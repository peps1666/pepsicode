...
def temp_session_dir(tmp_path):
    ...
    sessions_dir.mkdir()
    with patch("pepsicode.session.SESSIONS_DIR", sessions_dir), \
...
def test_load_session_accepts_unique_short_prefix(temp_session_dir):
    """Test loading a session from the short id shown in the UI."""
    session = create_new_session(workspace="/tmp/test")
    session.messages = [{"role": "user", "content": "Hello"}]
    save_session(session)

    short_id = session.session_id[:8]
    assert resolve_session_id(short_id) == session.session_id

    loaded = load_session(short_id)
    assert loaded is not None
    assert loaded.session_id == session.session_id
