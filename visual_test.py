"""Visual test 鈥?render each TUI component to verify the enhanced aesthetics."""
...
print("=" * 60)
print("  VISUAL TEST 鈥?Enhanced TUI Components")
print("=" * 60)
print()
...
entries = [
    ...
    TranscriptEntry(
        id=2,
        kind="assistant",
        body="I'll look at the auth module and fix the login issue.\n\n**Key changes:**\n1. Fixed token validation\n2. Added error handling\n\n```python\ndef login(user, pwd):\n    token = validate(user, pwd)\n    return token\n```",
    ),
    TranscriptEntry(
        id=3,
        kind="tool",
        body="File edited successfully",
        toolName="edit_file",
        status="success",
        collapsed=True,
        collapsedSummary="auth.py +5 -2",
    ),
